...
extension UINavigationController : UIGestureRecognizerDelegate
{
	...
	@objc func panningEdge(gesture: UIScreenEdgePanGestureRecognizer)
	{
		...
		func panning()
		{
			...
			{
				var currentState = 0
				if (pointX < Constants.edgeMinmalCommitWidth && indicator.cornerType == .left) || (pointX > -Constants.edgeMinmalCommitWidth && indicator.cornerType == .right)
				{
				...
				} else
				{
					...
					{
					case .left:
						currentState = Int((pointX - Constants.edgeMinmalCommitWidth) / 64) + 1
						if pointX > indicator.maxiumumWidth + Constants.edgeMinmalCommitWidth { pointX = indicator.maxiumumWidth + Constants.edgeMinmalCommitWidth }
						indicator.transform = CGAffineTransform.init(translationX: pointX, y: 0)
						break
					case .right:
						currentState = Int((fabs(pointX) + Constants.edgeMinmalCommitWidth) / 64) - 1
						if fabs(pointX) > indicator.maxiumumWidth + Constants.edgeMinmalCommitWidth { pointX = -indicator.maxiumumWidth - Constants.edgeMinmalCommitWidth }
						indicator.transform = CGAffineTransform.init(translationX: pointX, y: 0)
					}
		...
		func endPanning()
		{
			guard let indicator = currentEdgeIndicator else { return }
			var triggered = false
			if indicator.cornerType == .left && pointX >= Constants.edgeMinmalCommitWidth { triggered = true } else if indicator.cornerType == .right && pointX <= -Constants.edgeMinmalCommitWidth { triggered = true }
