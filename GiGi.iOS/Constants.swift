//
//  Constants.swift
//  GiGi
//
//  Created by Sean Cheng on 28/06/2017.
//  Copyright © 2017 Zheng Xingzhi. All rights reserved.
//

import UIKit

struct Constants
{
	 static let statusBarHeight : CGFloat = 20
	 static let keyboardBarHeight : CGFloat = 48
	 static let cellHeight : CGFloat = 47
	 static let iconCellHeight : CGFloat = 44
	 static let topBarHeight : CGFloat = 49
	 static let thumbHeight : CGFloat = 255
	 static let edgeMinmalCommitWidth : CGFloat = 44
	 static let slideMinmalCommitWidth : CGFloat = 44
	 static let moveActivePercent : CGFloat = 0.18
	 static let moveDeactivePercent : CGFloat = 0.15
	 static let packageIconSize : CGFloat = 64
	 static let packageIconMargin : CGFloat = 10
	 static let packageViewInset : CGFloat = 10
	 static let edgeMargin : CGFloat = 10
	 static let searchBarHeight : CGFloat = 36
	 static let defaultCornerRadius : CGFloat = 8
	 static let iconCornerRadius : CGFloat = 13
}
